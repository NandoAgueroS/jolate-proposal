...
function initPaperForm(opts) {
  ...
  const fileEmptyText = paperForm.querySelector(
    "#" + idPrefix + "-file-empty-text",
  );
  const fileNameDisplay = paperForm.querySelector(
    "#" + idPrefix + "-file-name-display",
  );
  const fileSelectedCheck = paperForm.querySelector(
    "#" + idPrefix + "-file-selected-check",
  );
  ...
  if (fileEmptyText) {
    const setFileButtonLabel = () => {
      fileEmptyText.textContent = t("enviar.file_button").replace(
        "{max}",
  ...
  function setFieldInvalid(fieldName, invalid) {
    const input = paperForm.querySelector(
      "#" + idPrefix + "-" + (idMap[fieldName] || fieldName),
    );
    if (!input) return;
  ...
  function showFieldError(fieldName, message) {
    const span = paperForm.querySelector(
  ...
  function getSelectedRole() {
    ...
    for (const radio of roleRadios) {
      if (radio.checked)
        return radio.value === "Asistente" ? "Asistente" : "Expositor";
    }
    return "Expositor";
  }
  ...
  if (fileInput) {
    fileInput.addEventListener("change", () => {
      ...
      if (file.size > maxFileSizeBytes) {
        showFieldError(
          "archivo",
          t("enviar.error_size").replace("{max}", String(maxFileSizeMB)),
        );
        resetFileInputState();
        return;
      }
  ...
  function validateSingleField(field) {
    ...
    if (
      field.type === "email" &&
      (value.length > 200 || !emailRe.test(value))
    ) {
  ...
  function setupLiveValidation() {
    ...
    liveFields.forEach((field) => {
      ...
      const markTouched = () => {
        field.dataset.jolateTouched = "true";
      };
      ...
      field.addEventListener("input", () => {
        const span = paperForm.querySelector(
          '.field-error[data-field="' + field.name + '"]',
        );
  ...
  paperForm.addEventListener("submit", (e) => {
    ...
    function setLoading(loading) {
      ...
      submitBtn.innerHTML = loading
        ? '<svg class="animate-spin w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">' +
          ...
          t("enviar.sending") +
          "</span>"
        : '<i data-lucide="send" class="w-5 h-5"></i><span>' +
          t("enviar.submit") +
          "</span>";
    ...
    xhr.onreadystatechange = () => {
      ...
      try {
        ...
        showGeneralError(
          xhr.status >= 500
            ? t("enviar.error_server")
            : t("enviar.error_unexpected"),
        );
        return;
      }
      ...
      if (resp.success) {
        ...
        paperForm.reset();
        if (roleRadios.length) {
          const radio = paperForm.querySelector(
            'input[name="rol"][value="' + savedRole + '"]',
          );
