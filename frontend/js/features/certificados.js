...
export function initCertificadosModal() {
  ...
  function setMessage(html, tone) {
    ...
    resultBox.innerHTML =
      '<div class="' + cls + ' flex items-start gap-2"><i data-lucide="' +
      (tone === 'error' ? 'alert-circle' : 'check-circle') +
      '" class="w-5 h-5 shrink-0 mt-0.5"></i><span>' + html + '</span></div>';
    refreshIcons();
  }
  ...
  function setLoading(loading) {
    ...
    if (submitLabel) {
      submitLabel.textContent = loading ? t('certificados.buscando') : t('certificados.buscar');
    }
  }
  ...
  function renderResultados(dni, registros, banner) {
    ...
    const items = registros.map((reg) => {
      const yaDescargado = reg.certificado
        ? '<div class="mt-2 text-xs text-text/60">' + t('certificados.ya_descargado') + '</div>'
        : '';
      return (
        '<div class="rounded-xl border border-tint/60 bg-bg p-4">' +
          '<div class="font-semibold text-text">' + escapeHtml(reg.nombre) + '</div>' +
          '<div class="text-xs text-text/70 mt-0.5">' + escapeHtml(reg.rol || '') + '</div>' +
          ...
          'data-descargar-dni="' + escapeAttr(dni) + '" data-descargar-id="' + escapeAttr(reg.id) + '">' +
            '<i data-lucide="download" class="w-4 h-4"></i><span>' + t('certificados.descargar') + '</span>' +
          '</button>' +
          yaDescargado +
        '</div>'
      );
    ...
    resultBox.innerHTML =
      '<div class="space-y-3">' +
      (banner ? '<div class="text-text/80 flex items-start gap-2"><i data-lucide="check-circle" class="w-5 h-5 shrink-0 mt-0.5"></i><span>' + banner + '</span></div>' : '') +
      items.join('') +
      '</div>';
    refreshIcons();
    ...
    resultBox.querySelectorAll('[data-descargar-dni]').forEach((btn) => {
      btn.addEventListener('click', () => {
        descargar(btn.getAttribute('data-descargar-dni'), btn.getAttribute('data-descargar-id'), btn);
      });
    });
  }
  ...
  function descargar(dni, id, btn) {
    ...
    fetch('certificado.php?action=descargar&dni=' + encodeURIComponent(dni) + '&id=' + encodeURIComponent(id), {
    ...
    fetch(form.getAttribute('action') + '?action=buscar', {
      ...
      .then((data) => {
        if (data && data.success) {
          ...
          setMessage((data && data.error) || t('certificados.error_generico'), 'error');
        }
      })
      .catch(() => {
        setMessage(t('certificados.error_conexion'), 'error');
