...
export function initProgramTabs() {
  ...
  function getRowStyle(ev) {
    ...
    if (type === "receso" || type === "break")
      return {
        typeCls: "text-white/40",
        dot: "bg-white/20",
        icon: "utensils-crossed",
      };
    if (type.indexOf("keynote") !== -1)
      return { typeCls: "text-accent font-bold", dot: "bg-accent", icon: null };
    if (
      type.indexOf("cient") !== -1 ||
      type.indexOf("scientific") !== -1 ||
      type.indexOf("sesion") !== -1 ||
      type.indexOf("session") !== -1
    )
      return { typeCls: "text-white/70", dot: "bg-white/40", icon: null };
    if (type.indexOf("mesa") !== -1 || type.indexOf("expert") !== -1)
      return { typeCls: "text-accent/80", dot: "bg-accent/60", icon: null };
    if (
      type.indexOf("foro") !== -1 ||
      type.indexOf("research") !== -1 ||
      type.indexOf("forum") !== -1
    )
      return { typeCls: "text-white/70", dot: "bg-white/40", icon: null };
    if (
      type.indexOf("apertura") !== -1 ||
      type.indexOf("ceremonia") !== -1 ||
      type.indexOf("opening") !== -1 ||
      type.indexOf("ceremony") !== -1
    )
      return { typeCls: "text-white font-bold", dot: "bg-white", icon: null };
    if (type.indexOf("cierre") !== -1 || type.indexOf("closing") !== -1)
      return { typeCls: "text-white font-bold", dot: "bg-white", icon: null };
  ...
  function renderDayEvents(dayIdx) {
    ...
    const comingSoonText =
      getLang() === "es" ? "[Pr\u00f3ximamente]" : "[Coming Soon]";
    ...
    for (const ev of day.events) {
      ...
      if (isBreak) {
        ...
        parts.push(
          '<div class="flex gap-4 sm:gap-5 px-5 sm:px-8 py-3 ' +
            breakBg +
            '">' +
            '<div class="w-24 sm:w-28 shrink-0 pt-px">' +
            '<span class="font-mono text-sm font-medium text-white/50">' +
            escapeHtml(ev.time) +
            "</span>" +
            "</div>" +
            ...
            '<i data-lucide="' +
            style.icon +
            '" class="w-4 h-4 text-white/30"></i>' +
            '<span class="text-white/60 text-sm font-light">' +
            escapeHtml(ev.title) +
            "</span>" +
            "</div>" +
            "</div>",
        ...
        const durHtml =
          parsed && parsed.duration > 0 && !isNaN(parsed.duration)
        ...
        const comingBadge = isCompletar
          ? '<span class="text-[14px] font-mono font-bold px-1.5 py-0.5 rounded-full shrink-0 bg-white/10 text-white/50">' +
            comingSoonText +
            "</span>"
          : "";

        parts.push(
          '<div class="flex gap-4 sm:gap-5 px-5 sm:px-8 py-4 ' +
            rowBg +
            '">' +
