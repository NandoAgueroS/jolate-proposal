...
const API = {
  me: () =>
    fetch('admin/auth.php?action=me', { credentials: 'same-origin' }).then(r => r.json()),
  login: (user, password) =>
  ...
  detail: (id) =>
    fetch('admin/detail.php?id=' + encodeURIComponent(id), { credentials: 'same-origin' })
      .then(r => r.json()),
};
...
function renderLogin(errorMsg) {
  ...
  const card = el('div', { class: 'min-h-screen flex items-center justify-center px-4' }, [
    el('div', { class: 'w-full max-w-md bg-white rounded-xl shadow-lg border border-tint p-8' }, [
      el('div', { class: 'flex items-center gap-3 mb-6' }, [
        el('div', { class: 'w-10 h-10 rounded-lg bg-primary text-white flex items-center justify-center font-display text-xl' }, 'J'),
        el('div', null, [
          el('div', { class: 'font-display text-xl text-primary font-semibold' }, 'JOLATE 2026'),
          el('div', { class: 'text-sm text-text/70' }, 'Panel de administración'),
        ]),
      ]),
      el('h1', { class: 'text-lg font-semibold text-text mb-1' }, 'Iniciar sesión'),
      el('p', { class: 'text-sm text-text/70 mb-6' }, 'Acceso restringido al organizador del evento.'),
      errorMsg
        ? el('div', { class: 'mb-4 p-3 rounded-md bg-red-50 border border-red-200 text-sm text-red-800' }, errorMsg)
        : null,
      el('form', { id: 'login-form', class: 'space-y-4', on: { submit: onLogin } }, [
        el('div', null, [
          el('label', { for: 'user', class: 'block text-sm font-medium text-text mb-1' }, 'Usuario'),
          el('input', {
            id: 'user', name: 'user', type: 'text', required: 'required',
            autocomplete: 'username',
            class: 'w-full rounded-md border border-tint px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent',
          }),
        ]),
        el('div', null, [
          el('label', { for: 'password', class: 'block text-sm font-medium text-text mb-1' }, 'Contraseña'),
          el('input', {
        ...
        el('button', {
          type: 'submit',
          class: 'w-full bg-primary hover:bg-accent text-white font-semibold rounded-md px-4 py-2.5 transition',
        }, 'Entrar'),
      ]),
    ]),
  ]);
...
async function onLogout() {
  try { await API.logout(); } catch (e) { /* swallow */ }
  if (state.dt) {
    state.dt.destroy();
    state.dt = null;
  }
...
function renderDashboard() {
  ...
  const root = el('div', { class: 'min-h-screen flex flex-col' }, [
    el('nav', { class: 'bg-primary text-white' }, [
      el('div', { class: 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between' }, [
        el('div', { class: 'flex items-center gap-3' }, [
          el('div', { class: 'w-8 h-8 rounded bg-white/10 flex items-center justify-center font-display' }, 'J'),
          el('div', { class: 'font-semibold' }, 'Admin · Inscriptos JOLATE 2026'),
        ]),
        el('div', { class: 'flex items-center gap-2' }, [
          el('a', {
            id: 'export-csv', href: 'admin/export.php',
    ...
    el('main', { class: 'flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6' }, [
      el('div', { class: 'bg-white rounded-xl border border-tint p-4 mb-4 flex flex-wrap items-center gap-3' }, [
        el('label', { for: 'rol-filter', class: 'text-sm font-medium text-text' }, 'Rol:'),
        el('select', {
          id: 'rol-filter',
          class: 'rounded-md border border-tint px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent',
        }, [
          el('option', { value: '' }, 'Todos'),
          el('option', { value: 'Expositor' }, 'Expositor'),
          el('option', { value: 'Asistente' }, 'Asistente'),
        ]),
        el('span', { class: 'text-xs text-text/60 ml-auto' }, 'Búsqueda global en la barra superior de la tabla.'),
      ]),
      el('div', { class: 'bg-white rounded-xl border border-tint p-2 sm:p-4 overflow-x-auto' }, [
        el('table', { id: 'inscriptos-table', class: 'display w-full text-sm' }, [
          el('thead', null, [
            el('tr', null, [
              el('th', null, 'ID'),
              el('th', null, 'Rol'),
    ...
    el('div', {
      ...
      el('div', { class: 'bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto' }, [
        el('div', { class: 'flex items-center justify-between p-4 border-b border-tint' }, [
          el('h2', { class: 'text-lg font-semibold text-text' }, 'Detalle del inscripto'),
          el('button', {
            id: 'detalle-close', type: 'button',
  ...
  document.getElementById('export-csv').addEventListener('click', (ev) => {
    ...
    window.location = 'admin/export.php' + (r ? ('?rol=' + encodeURIComponent(r)) : '');
  });
...
async function openDetail(id) {
  ...
  fields.forEach(([k, v]) => {
    ...
    row.className = 'grid grid-cols-3 gap-2 py-2 border-b border-tint/60 last:border-0';
    row.innerHTML = '<div class="text-text/60 font-medium">' + escapeHtml(k) + '</div>'
  ...
  if (d.tiene_pdf) {
    ...
    link.className = 'inline-flex items-center gap-2 mt-4 bg-primary hover:bg-accent text-white px-4 py-2 rounded-md text-sm font-medium';
    link.innerHTML = '<i data-lucide="download" class="w-4 h-4"></i>Descargar PDF';
